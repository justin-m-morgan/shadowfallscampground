defmodule ShadowfallscampgroundWeb.ReserveLive.Steps.Summary do
  ...
  prop tent_details_changeset, :struct, required: true
  ...
  def render(assigns) do
    ...
    <div>
      {#if any_errors?(assigns)}
        ...
        <ul>
          {#for {label, _changeset} <- invalid_changesets(assigns)}
            <li>{label}</li>
          {/for}
        </ul>
      {#else}
